  ...
  $(function () {
    ...
    Drupal.jsAC.prototype.found = function (matches) {
      ...
      for (var key in matches) {
        $('<li></li>')
          .html($('<a href="#"></a>').html(matches[key]).click(function (e) {
            e.preventDefault();
          }))
          .mousedown(function () {
            ac.select(this);
    ...
    Drupal.jsAC.prototype.setStatus = function (status) {
      var iconKey = Drupal.settings.druru.icons.key,
        ...
        $throbber = $('.' + iconThrobber, $('#' + this.input.id).parent());
      
      switch (status) {
        case 'begin':
          $throbber.addClass(iconKey + '-spin');
          $(this.ariaLive).html(Drupal.t('Searching for matches...'));
          break;
        case 'cancel':
        case 'error':
        case 'found':
          $throbber.removeClass(iconKey + '-spin');
          break;
      }
    };
  });
