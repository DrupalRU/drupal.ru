...
Calendar = (function() {
  var rjust_2_0, strftime_Ymd;
  function Calendar(_arg) {
    var MS_IN_DAY, color, container, coord, data, date, datestr_coord_map, day, dayHolder, day_size, days, days_until_next_sunday, i, interval, intervals, key, maxInterval, num_weeks, points, t, x, y, _i, _len, _len2, _ref, _ref2, _ref3;
    container = _arg.container, color = _arg.color, data = _arg.data, intervals = _arg.intervals, day_size = _arg.day_size, num_weeks = _arg.num_weeks;
    Calendar.__super__.constructor.call(this, 'div', 'Calendar');
    if (container) {
    ...
    for (x = _ref = num_weeks - 1; _ref <= 0 ? x <= 0 : x >= 0; _ref <= 0 ? x++ : x--) {
      for (y = _ref2 = 7 - 1; _ref2 <= 0 ? y <= 0 : y >= 0; _ref2 <= 0 ? y++ : y--) {
        datestr_coord_map[strftime_Ymd(t)] = "" + x + ":" + y;
        t = new Date(t.getTime() - MS_IN_DAY);
      }
    }
    days = {};
    for (x = 0; 0 <= num_weeks ? x < num_weeks : x > num_weeks; 0 <= num_weeks ? x++ : x--) {
      for (y = 0; y < 7; y++) {
        day = D();
        day.setStyles({
          width: day_size + 'px',
  ...
  Calendar.prototype.makeHolder = function(data, day_size, num_weeks) {
    ...
    this.appendChild(daysHolder);
    return daysHolder;
  };
  rjust_2_0 = function(s) {
    if (s.length === 1) {
      return "0" + s;
    } else {
      return s;
