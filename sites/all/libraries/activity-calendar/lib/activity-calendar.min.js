...
"BLR BL"),q("td","BM",[" "]),q("td","BLR BR")])])]);this.on("click",function(i){return j.click(i)});f=null;d={};h=0;for(k=a.length;h<k;h++){g=a[h];c=typeof g;if(c==="string")f=g;else if(c==="function")this.onclick=g;else _.extend(d,g)}if(d.id)this._.id=d.id;if(f)this.overlay.appendChild(this.label=w("Blabel",[f]))}A(b,s);b.prototype.setText=function(a){return this.label.setTextChild(a)};b.prototype.click=function(a){a&&a.stop();if(this.onclick)return this.onclick(a)};b.prototype.showLoader=function(){};
b.prototype.hideLoader=function(){};return b})();y=function(b){return b<16?"0"+b.toString(16):b.toString(16)};z=function(b){return[parseInt(b.substr(1,2),16),parseInt(b.substr(3,2),16),parseInt(b.substr(5,2),16)]};I=function(b,a,d){return"#"+y(b)+y(a)+y(d)};F=function(b,a,d){var c,f,g,h;c=z(b);g=c[0];b=c[1];c=c[2];f=z(a);h=f[0];a=f[1];f=f[2];return I(Math.round(g+(h-g)*d),Math.round(b+(a-b)*d),Math.round(c+(f-c)*d))};E=function(){function b(a,d,c){var f=this;this.duration=a;this.transition=d;this.f=
c;this.start=(new Date).getTime();this.f(0);setTimeout(function(){return f.step()},0)}b.prototype.step=function(){var a,d=this;a=((new Date).getTime()-this.start)/this.duration;if(a>1)a=1;this.f(this.transition(a));if(a<1)return setTimeout(function(){return d.step()},0)};return b}();(function(){return function(b,a){var d,c,f,g,h,k,j,i;c=a.duration;i=a.transition;g=a.oncomplete;h=a.opacity;j=a.rect;k=a.pos;d=a.background;b instanceof Array||(b=[b]);f=0;new E(c,i,function(o){var l,m,v;f++;m=0;for(v=
b.length;m<v;m++){l=b[m];h&&l.setOpacity(h[0]+(h[1]-h[0])*o);j&&l.setRect(H(j[0],j[1],o));k&&l.setPos(G(k[0],k[1],o));if(d)l._.style.background=F(d[0],d[1],o)}if(o===1&&g)return g({numFrames:f,info:a})})}})();x=function(b){var a,d,c,f;a=typeof b;if(b===null||a==="undefined")return"null";else if(a==="boolean"||a==="number")return b.valueOf();else if(a==="string")return'"'+b.replace(/\\/g,"\\\\").replace(/"/g,'\\"').replace(/\n/,"\\n")+'"';else if(b instanceof Array){a=[];c=0;for(f=b.length;c<f;c++){d=
b[c];a.push(x(d))}return"["+a.join(",")+"]"}else{a=[];for(d in b)if(r.call(b,d)){c=b[d];a.push(x(d)+":"+x(c))}return"{"+a.join(",")+"}"}};var J;r=Object.prototype.hasOwnProperty;A=function(b,a){function d(){this.constructor=b}for(var c in a)if(r.call(a,c))b[c]=a[c];d.prototype=a.prototype;b.prototype=new d;b.__super__=a.prototype;return b};J=function(){function b(c){var f,g,h,k,j,i,o,l,m,v,p,n,B,K;g=c.container;f=c.color;k=c.data;p=c.intervals;j=c.day_size;n=c.num_weeks;b.__super__.constructor.call(this,
"div","Calendar");g&&document.getElementById(g).appendChild(this._);p=p||[4,10,15,20];c=p[p.length-1];o=this.makeHolder(k,j,n);g={};l=new Date;i=l.getDay();h=6-(7+i-1)%7;l=new Date(l.getTime()+864E5*h);for(h=i=n-1;i<=0?h<=0:h>=0;i<=0?h++:h--)for(m=6;m>=0;m--){g[d(l)]=""+h+":"+m;l=new Date(l.getTime()-864E5)}l={};for(h=0;0<=n?h<n:h>n;0<=n?h++:h--)for(m=0;m<7;m++){i=w();i.setStyles({width:j+"px",height:j+"px",background:"#ebedf0",position:"absolute",left:1+(j+2)*h+"px",top:1+(j+2)*m+"px",zIndex:2});
l[""+h+":"+m]=i;o.appendChild(i)}n=0;for(o=k.length;n<o;n++){h=k[n];j=h[0];B=h[1];h=g[j];i=0;for(K=p.length;i<K;i++){m=p[i];if(m<B)v=p[i+1]}h&&l[h].setStyles({background:f,opacity:1/c*v});l[h]._.setAttribute("title",j+", "+B+" points")}}var a,d;A(b,s);b.prototype.getColor=function(){};b.prototype.makeHolder=function(c,f,g){c=f*g+1*(g+1);g=f*7+8;f=w();f.setStyles({width:c+"px",height:g+"px",position:"relative",background:"#FFFFFF"});this.appendChild(f);return f};a=function(c){return c.length===1?"0"+
c:c};d=function(c){var f,g;f=1900+c.getYear();g=a(""+(c.getMonth()+1));c=a(""+c.getDate());return""+f+"-"+g+"-"+c};return b}();window.Calendar=J})();
