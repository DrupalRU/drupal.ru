(function ($, Drupal) {
  ...
  /**
   * Provide vertical tab summaries for Druru settings.
   */
  Drupal.behaviors.druruSettingSummaries = {
    attach: function (context) {
      ...
      $context.find('#edit-components').drupalSetSummary(function () {
        var summary = [];
        // Breadcrumbs.
        var breadcrumb = parseInt($context.find('select[name="druru_breadcrumb"]').val(), 10);
        if (breadcrumb) {
          summary.push(Drupal.t('Breadcrumbs'));
        }
        // Navbar.
        var navbar = 'Navbar: ' + $context.find('select[name="druru_navbar_position"] :selected').text();
        if ($context.find('input[name="druru_navbar_inverse"]').is(':checked')) {
      ...
      $context.find('#edit-javascript').drupalSetSummary(function () {
        var summary = [];
        if ($context.find('input[name="druru_anchors_fix"]').is(':checked')) {
          summary.push(Drupal.t('Anchors'));
        }
        if ($context.find('input[name="druru_popover_enabled"]').is(':checked')) {
          summary.push(Drupal.t('Popovers'));
        }
        if ($context.find('input[name="druru_tooltip_enabled"]').is(':checked')) {
      ...
      $context.find('#edit-advanced').drupalSetSummary(function () {
        var summary = [];
        // Bootstrap CDN.
        var bootstrapCDN = $context.find('select[name="druru_bootstrap_cdn"]').val();
        if (bootstrapCDN.length) {
          bootstrapCDN = 'BootstrapCDN v' + bootstrapCDN;
          // Bootswatch.
          if ($context.find('select[name="druru_bootswatch"]').val().length) {
            bootstrapCDN += ' (' + $context.find('select[name="druru_bootswatch"] :selected').text() + ')';
          }
          summary.push(bootstrapCDN);
        }
        // Rebuild registry.
        if ($context.find('input[name="druru_rebuild_registry"]').is(':checked')) {
   ...
   */
  Drupal.behaviors.druruBootswatchPreview = {
    attach: function (context) {
      var $context = $(context);
      ...
      $preview.once('bootswatch', function () {
        $.get("http://api.bootswatch.com/3/", function (data) {
          ...
          $preview.parent().find('select[name="druru_bootswatch"]').bind('change', function () {
   ...
   */
  Drupal.behaviors.druruNavbarPreview = {
    attach: function (context) {
      var $context = $(context);
      ...
      $preview.once('navbar', function () {
        ...
        $preview.find('select[name="druru_navbar_position"]').bind('change', function () {
        ...
        $preview.find('input[name="druru_navbar_inverse"]').bind('change', function () {
